...
export function MessageWidget({ className }) {
    ...
    return (
        <div className={cn("bg-bg-panel border border-border-color rounded-lg p-4 flex flex-col", className)}>
            <h3 className="text-text-muted text-sm font-semibold uppercase tracking-wider mb-1">Rolling Message</h3>
            <div className="flex-1 flex items-center justify-between px-4 bg-bg-dark rounded border border-white/5">
                <span className="text-xl font-bold text-accent-primary font-mono">{label}</span>
                <span className="text-xl font-bold text-white font-mono">{cleanValue}</span>
            </div>
        </div>
    );
}
